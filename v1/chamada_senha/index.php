...
if (isset($_GET["columns"])) {
    ...
    if (isset($dados['basic'])) {
        ...
        foreach ($body_columns as $key => $column) {
            if ($column == 'id_status') {
                $body_columns[$key] =  "agenda.$column";
                if (strpos($join, 'agenda on') === false) {
                    $join .= "LEFT join agenda on agenda.id_agenda = $table.id_agenda";
                }
            } else {
    ...
    foreach ($dados as $key => $value) {
        ...
        if ($key == "######") {
            if (strpos($value, ',') === false) {
                $body_where[] = "######." . $key . " :" . str_replace($cond, "", $key);
                ...
                $body_where[] = "######." . $key . " in (" . implode(",", $nova_key) . ")";
            }
